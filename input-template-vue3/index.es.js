import { ref as E, onMounted as V, onUnmounted as M, watch as W, openBlock as j, createElementBlock as z, withKeys as F } from "vue";
const G = (L, S) => {
  ...
  emits: ["change", "submit"],
  setup(L, { expose: S, emit: N }) {
    const x = L, k = N, i = E(null), A = E("");
    E([]), E(-1);
    const D = (e) => {
      ...
      const n = (s) => {
        ...
        if (s.tagName === "INPUT") {
          const u = (d = i.value) == null ? void 0 : d.querySelector(`[data-id="${(h = s.closest("[data-id]")) == null ? void 0 : h.dataset.id}"] input`);
      ...
      if (!((e.ctrlKey || e.metaKey) && e.key === "z")) {
        if (e.key === "Backspace" || e.key === "Delete") {
          ...
          if (h) {
            if (e.key === "Backspace" && (d.textContent.length === 1 || o.startOffset === 1 && o.startOffset === o.endOffset) || e.key === "Delete" && (d.textContent.length === 1 || o.startOffset === 0 && o.startOffset === o.endOffset && d.textContent.length === 1)) {
              ...
              c.setStart(d, 0), c.collapse(!0), l.removeAllRanges(), l.addRange(c), b(i.value.innerHTML);
              return;
            }
            if (d.textContent.trim() === "" && d.classList.contains("showing-placeholder") && (e.key === "Backspace" || e.key === "Delete")) {
              setTimeout(() => {
                ...
                if (c && c.parentNode) {
                  const v = window.getSelection(), f = document.createRange();
                  f.selectNode(c), v.removeAllRanges(), v.addRange(f), document.execCommand("insertText", !1, "");
                  const w = document.createTextNode("​");
                  c.parentNode.insertBefore(w, c.nextSibling), c.remove();
                  const m = document.createRange();
                  m.setStart(w, 0), m.collapse(!0), v.removeAllRanges(), v.addRange(m), document.execCommand("insertText", !1, ""), b(i.value.innerHTML);
                }
              }, 0);
              return;
            }
          ...
          if (e.key === "Backspace" && r.nodeType === Node.TEXT_NODE && p === 0) {
            ...
            for (; g && g.nodeType === Node.TEXT_NODE && g.textContent.trim() === ""; )
              g = g.previousSibling;
            if (g && g.nodeType === Node.ELEMENT_NODE && (g.classList.contains("input-wrapper") || g.classList.contains("select-wrapper"))) {
              const c = g.querySelector(".editable-div");
              if (c && c.textContent.trim() === "" && c.classList.contains("showing-placeholder")) {
                e.preventDefault();
                const f = g, w = window.getSelection(), m = document.createRange();
                m.selectNode(f), w.removeAllRanges(), w.addRange(m), document.execCommand("insertText", !1, "");
                const T = document.createTextNode("​");
                f.parentNode.insertBefore(T, f.nextSibling), f.remove();
                const y = document.createRange();
                y.setStart(T, 0), y.collapse(!0), w.removeAllRanges(), w.addRange(y), document.execCommand("insertText", !1, ""), b(i.value.innerHTML);
                return;
              }
              setTimeout(() => {
                ...
                if (!f.rangeCount)
                  return;
                let m = f.getRangeAt(0).startContainer, T = !1, y = null;
                for (; m && m !== i.value; ) {
                  if (m.classList && m.classList.contains("editable-div")) {
                    T = !0, y = m;
                    break;
                  }
                  m = m.parentNode;
                ...
                if (T) {
                  const C = y.closest(".input-wrapper");
                  if (C) {
                    document.execCommand("insertText", !1, "");
                    const $ = document.createTextNode("​");
                    C.parentNode.insertBefore($, C.nextSibling);
                    const O = document.createRange();
                    O.setStart($, 0), O.collapse(!0), f.removeAllRanges(), f.addRange(O), document.execCommand("insertText", !1, "");
                  }
                }
                b(i.value.innerHTML);
              }, 0);
          ...
          if (e.key === "Delete" && (r.nodeType === Node.TEXT_NODE && p === r.textContent.length || r.nodeType === Node.TEXT_NODE && r.textContent.trim() === "")) {
            ...
            if (c && c.nodeType === Node.ELEMENT_NODE && c.classList.contains("input-wrapper")) {
              ...
              if (v) {
                ...
                const f = window.getSelection(), w = document.createRange();
                v.firstChild && v.firstChild.nodeType === Node.TEXT_NODE ? w.setStart(v.firstChild, 0) : w.setStart(v, 0), w.collapse(!0), f.removeAllRanges(), f.addRange(w), console.log("Del 键处理: 光标已设置到 editable-div", {
                  editableDiv: v,
                  focused: document.activeElement === v
                }), setTimeout(() => {
                  document.activeElement !== v && c.setAttribute("contenteditable", "true");
                }, 300), b(i.value.innerHTML);
                return;
              }
            }
          }
        ...
        if (e.key === "ArrowLeft" || e.key === "ArrowRight" || e.key === "ArrowUp" || e.key === "ArrowDown") {
          ...
          if (r) {
            e.preventDefault();
            const h = r.closest(".select-wrapper"), d = document.createRange();
            e.key === "ArrowRight" ? d.setStartAfter(h) : e.key === "ArrowLeft" && d.setStartBefore(h), d.collapse(!0), l.removeAllRanges(), l.addRange(d);
            return;
          }
    ...
    function H() {
      ...
      x.elements.forEach((a) => {
        const n = P(a);
        e += n + " ";
      }), i.value.innerHTML = e, R.value = D(e);
    }
    V(() => {
      x.showTemplate && H(), i.value.addEventListener("input", (n) => {
        const t = n.target;
        (t.tagName === "SELECT" || t.tagName === "INPUT" || t.classList.contains("editable-div")) && b(i.value.innerHTML);
        ...
        if (t) {
          ...
          if (l.classList.toggle("open"), l.classList.contains("open")) {
            ...
            if (o) {
              ...
              const p = o.offsetHeight || 200, h = window.innerHeight;
              r.bottom + p > h && (o.style.top = `${r.top - p}px`);
            }
          }
        }
        ...
        if (s) {
          ...
          }), s.classList.add("selected"), o.textContent = r, o.classList.remove("showing-placeholder"), l.classList.remove("open"), b(i.value.innerHTML);
        }
      ...
      console.log("newVal", e), x.showTemplate && H();
    });
    const P = (e) => {
      if (e.type === "plaintext")
        return e.value;
    ...
    S({
      clearInput: () => {
        i.value.innerHTML = "", A.value = "", R.value = "", b("");
      },
      initialText: R
    });
    const U = () => {
...
`, document.head.appendChild(_), (e, a) => (j(), z("div", {
      class: "rich-editor",
      ref_key: "editor",
      ref: i,
...
}, Z = /* @__PURE__ */ G(J, [["__scopeId", "data-v-4606cd6a"]]);
export {
  Z as default
};
